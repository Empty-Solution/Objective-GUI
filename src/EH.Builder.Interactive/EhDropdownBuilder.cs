...
using DK.Binding.Abstraction.Generic;
...
public class EhDropdownBuilder(IEhVisualOption visual)
{
    ...
    private readonly EhTextBuilder                      m_TextBuilder              = new(visual);
    ...
    public IOgContainer<IOgElement> Build(string name, IDkProperty<int> selected, string[] values, EhOptionsProvider provider)
    {
        ...
        container.Add(m_TextBuilder.BuildStaticText($"{name}NameText", option.TextColor, name, 
            option.TextNameFontSize, option.TextNameAlignment, provider.InteractableElementOption.Width - option.Width, provider.InteractableElementOption.Height));
        ...
        IOgContainer<IOgElement> sourceContainer = m_ContainerBuilder.Build($"{name}SourceContainer",
            new OgScriptableBuilderProcess<OgContainerBuildContext>(context =>
            {
                context.RectGetProvider.Options.SetOption(new OgSizeTransformerOption(option.Width, (option.ModalItemHeight + option.ModalItemPadding) * values.Length))
                       .SetOption(new OgMarginTransformerOption(0, option.Height - option.ModalItemPadding));
            }));
        button.Add(new OgInteractableElement<IOgElement>($"{name}ModalInteractable", 
            new OgEventHandlerProvider(), new DkReadOnlyGetter<Rect>(new(0, 0, option.Width, (option.ModalItemHeight + option.ModalItemPadding) * values.Length))));
    ...
    private IOgContainer<IOgVisualElement> BuildDropdownItem(string name, int index, IDkProperty<int> selected, 
        DkObservableProperty<string> property, List<EhDropdownTextObserver> observers, IOgModalInteractable<IOgElement> interactable, EhOptionsProvider provider)
    {
        EhDropdownOption           option   = provider.DropdownOption;
        DkScriptableObserver<bool> observer = new();
        observer.OnUpdate += state =>
        ...
        EhDropdownTextObserver textObserver     = new(observers, observers.Count, option.ItemTextColor, option.SelectedItemTextColor, textGetter, binding, interactable);
        ...
        OgTextureElement background = m_BackgroundBuilder.Build($"{name}Background", backgroundGetter, 
            option.Width * 0.9f - option.Width * 0.05f, option.ModalItemHeight, option.Width * 0.05f, 0, new(option.Border, option.Border, option.Border, option.Border),
            context =>
            {
    ...
    private class EhDropdownTextObserver(List<EhDropdownTextObserver> observers, int index, IDkGetProvider<Color> color,
        IDkGetProvider<Color> selectedColor, OgAnimationColorGetter getter, IDkBinding binding, IOgModalInteractable<IOgElement> interactable) : IDkObserver<bool>
    {
        public void Update(bool state)
        {
            ...
            for(int i = 0; i < observers.Count; i++)
                observers[i].InternalUpdate(i == index);
