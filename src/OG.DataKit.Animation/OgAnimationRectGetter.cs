    ...
    : OgAnimationGetter<TGetter, Rect>(originalGetter, provider) where TGetter : IDkGetProvider<Rect>
{
    protected override Rect CalculateValue(Rect currentValue, Rect targetValue, float time) =>
        new(Mathf.Lerp(currentValue.x, targetValue.x, time),
            Mathf.Lerp(currentValue.y, targetValue.y, time),
            Mathf.Lerp(currentValue.width, targetValue.width, time),
            Mathf.Lerp(currentValue.height, targetValue.height, time));
