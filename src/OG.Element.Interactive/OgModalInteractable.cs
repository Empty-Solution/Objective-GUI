                                             ...
                                             IOgModalInteractable<TElement> where TElement : IOgElement
{
    ...
    public bool Invoke(IOgMouseKeyUpEvent reason)
    {
        ...
        if(!isHovering && !shouldProcess)
            return false; // проверка говна, но на любом сете шуд процесс пересортирует все элементы, а в меню модалок может быть до 30 шт, из-за чего возникают просадки по фпс.
