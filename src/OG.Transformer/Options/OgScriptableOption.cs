...
public class OgScriptableOption(string name) : IOgTransformerOption
{
    ...
    public object? GetProperty(string propertyName) => m_Properties.TryGetValue(propertyName, out object? property) ? property : null;
    public T? GetProperty<T>(string propertyName) => (T?)GetProperty(propertyName);
    public void SetProperty(string propertyName, object value)
    {
        if(m_Properties.ContainsKey(propertyName)) m_Properties[propertyName] = value;
