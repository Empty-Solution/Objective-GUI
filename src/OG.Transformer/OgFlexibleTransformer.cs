...
public class OgFlexibleTransformer : OgBaseTransformer<OgFlexibleTransformerOption>
{
    ...
    public override Rect Transform(Rect rect, Rect parentRect, Rect lastRect, int remaining, OgFlexibleTransformerOption option)
    {
        ...
        return option.Orientation == EOgOrientation.HORIZONTAL
                   ? new(rect.x + occupied, rect.y, Mathf.Max(free / remaining, 0) == 0 ? rect.width : Mathf.Max(free / remaining, 0), parentRect.height)
                   : new(rect.x, rect.y + occupied, parentRect.width, Mathf.Max(free / remaining, 0) == 0 ? rect.height : Mathf.Max(free / remaining, 0));
    }
}