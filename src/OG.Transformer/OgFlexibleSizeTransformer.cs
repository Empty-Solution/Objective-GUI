...
public class OgFlexibleSizeTransformer : OgBaseTransformer<OgFlexibleSizeTransformerOption>
{
    ...
    public override Rect Transform(Rect rect, Rect parentRect, Rect lastRect, int remaining, OgFlexibleSizeTransformerOption option)
    {
        ...
        float width = option.Orientation is EOgOrientation.HORIZONTAL or EOgOrientation.ALL 
            ? Mathf.Max(freeHorizontal / remaining, 0) : parentRect.width;
            
        float height = option.Orientation is EOgOrientation.VERTICAL or EOgOrientation.ALL
            ?  Mathf.Max(freeVertical / remaining, 0) : parentRect.height;
